...
class SelectAnimeDir(PopupMenu):
    ...
    def update_completion(self, value: str) -> None:
        ...
        rule_widget = self.query_one("#completions-rule-widget", Rule)
        completions_widget = self.query_one("#completions-widget", OptionList)
    ...
    def action_switch_completion(self, rev: bool = False) -> None:
        options_widget = self.query_one("#completions-widget", OptionList)
        if len(options_widget.options) > 0:
            ...
            if (index := options_widget.highlighted) is not None:
                ...
                input_widget = self.query_one("#anime-dir-input", Input)
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Rule(id="completions-rule-widget")

        completions_widget = OptionList(compact=True, id="completions-widget")
